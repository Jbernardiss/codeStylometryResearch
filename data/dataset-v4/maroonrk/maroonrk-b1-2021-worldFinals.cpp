...
struct Z{
	ull s;
	mint p;
	Z(ull ss=0,mint pp=1):s(ss),p(pp){}
	void rand_init(){
		s=rand_int(1,mint::mod-1);
		p=s;
	}
	void meld(const Z&z){
		s+=z.s;
...
void slv(){
	...
	Z allval;
	rep(i,b)allval.meld(rw[i]);
	rep(k,2){
		rep(i,s)ls[i]=rw[vs[k][i]];
		vc<Z> tmp(n);
		vvc<int> buf(B);
		rep(i,n){
			if(qs[i].l<=qs[i].r){
				if(qs[i].m<B){
					...
					for(int j=qs[i].l;j<=qs[i].r;j+=qs[i].m){
						tmp[i].meld(ls[j-1]);
					}
				}
			}
		}
		rng(w,1,B)if(si(buf[w])){
			rep(i,s){
				sum[i]=ls[i];
				if(i>=w)sum[i].meld(sum[i-w]);
			}
			for(auto i:buf[w]){
				tmp[i]=sum[qs[i].r-1];
				if(qs[i].l>w)tmp[i].meld(sum[qs[i].l-w-1].inv());
			}
		}
		auto unko=allval;
		rep(i,n){
			if(qs[i].w==1)unko.meld(tmp[i].inv());
			else unko.meld(tmp[i]);
			if(look.count(unko))ans[k][i]=look[unko];
		}
	}
	rep(i,n){
		pi p(ans[0][i],ans[1][i]);
		if(memo.count(p)){
			cout<<memo[p];
		}else{
			cout<<"X";
