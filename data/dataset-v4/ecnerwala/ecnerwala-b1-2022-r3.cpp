...
int main() {
	...
	for (int case_num = 1; case_num <= T; case_num ++) {
		...
		int N, C; cin >> N >> C;
		//cerr << "Case #" << case_num << ": " << N << ' ' << C << '\n';
		std::vector<std::array<int, 2>> bounds(C);
		for (auto& [x, y] : bounds) cin >> x >> y;
		std::vector<int> hats(N);
		for (auto& h : hats) { cin >> h; h--; }

		std::vector<std::vector<int>> locs(C);
		for (int i = 0; i < N; i++) locs[hats[i]].push_back(i);
		for (int i = 0; i < N; i++) locs[hats[i]].push_back(N+i);
		...
		auto update_node = [&](seg_tree::point a) {
			assert(seg[a].lazy >= 0);
			seg[a].val = seg[a].lazy ? 0 : (a >= layout.N ? 1 : seg[a[0]].val + seg[a[1]].val);
		};
		for (seg_tree::point a(layout.N*2 - 1); a > 0; a--) update_node(a);

		auto update_range = [&](int x, int y, int delta) {
			//cerr << "Update range " << x << ' ' << y << ' ' << delta << endl;
			auto rng = layout.get_range(x, y);
		...
		auto set_color = [&](int c, int st, int delta) -> void {
			//cerr << "Set color " << c << ' ' << st << ' ' << delta << endl;
			auto [a, b] = bounds[c];
			int idx = int(std::lower_bound(locs[c].begin(), locs[c].end(), st) - locs[c].begin());
			auto get_idx = [&](int o) {
				return (idx + o >= int(locs[c].size())) ? 2*N : locs[c][idx+o];
			};
			if (1 <= a) {
				update_range(get_idx(0), get_idx(a-1), delta);
			}
			update_range(get_idx(b), layout.N, delta);
		};
		for (int c = 0; c < C; c++) {
			set_color(c, 0, +1);
		}
		...
		int64_t ans = 0;
		for (int i = 0; i < N; i++) {
			update_range(0, i+1, 1);
			update_range(i+N-1, layout.N, 1);
			//cerr << "starting from " << i << ' ' << seg[1].val << '\n';
			ans += seg[1].val;
			update_range(0, i+1, -1);
			update_range(i+N-1, layout.N, -1);
