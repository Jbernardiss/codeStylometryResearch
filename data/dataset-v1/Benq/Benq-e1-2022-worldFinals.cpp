...
Line get_seg(int i, int j) {
	return {v.at(stk.at(i).at(j)), v.at(stk.at(i).at((j+1)%3))};
}

bool bad_pair(Line a, Line b) {
	if (sz(strictIsect(a,b))) return 1;
	if (sz(segIsect(a,b)) == 2) return 1;
	return 0;
}

bool check() {
	F0R(i,sz(stk)) F0R(j,3) F0R(k,3) {
		if (i == sz(stk)-1 && j == k) continue;
		if (bad_pair(get_seg(i, j), get_seg(sz(stk)-1, k))) return 0;
	}
	return 1;
}
...
void dfs(int used) {
	int idx = 0;
	while (idx < N && (used&(1<<idx))) ++idx;
	if (idx == N) {
		if (sz(stk) > sz(best)) best = stk;
		return;
	}
	FOR(a,idx+1,N) FOR(b,a+1,N) {
		int mask = (1<<idx)|(1<<a)|(1<<b);
		if (!(used&mask)) {
			stk.pb({idx,a,b});
			if (check()) {
				dfs(used|mask);
...
void solve(int tc) {
	...
	dfs(0);
	ps(sz(best));
	each(t,best) ps(1+t[0],1+t[1],1+t[2]);
}
