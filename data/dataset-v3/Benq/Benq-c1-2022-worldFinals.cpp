...
void solve(int tc) {
	...
	vi in(B), out(B);
	Dinic<int> D; D.init(2*B+2);
	vpi edges, edge_labels;
	rep(S) {
		ints(u,v); --u, --v;
		++out.at(u), ++in.at(v);
		edges.pb({u,v});
		edge_labels.pb(D.ae(u, v+B, S));
	}
	F0R(i,B) {
		D.ae(2*B, i, S-out.at(i));
		D.ae(i+B, 2*B+1, S-in.at(i));
	}
	if (D.maxFlow(2*B,2*B+1) + S < B*S) {
		ps("IMPOSSIBLE");
		return;
	}
	Euler<true> E; E.init(B);
	F0R(s,S) {
		int f = D.edgeFlow(edge_labels[s]);
		rep(f + 1) E.ae(edges[s].f, edges[s].s);
	}
	vpi v = E.solve();
	if (!sz(v)) {
		ps("IMPOSSIBLE");
