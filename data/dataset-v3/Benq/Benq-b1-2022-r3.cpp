...
void solve(int tc) {
	...
	// dbg("HUH", LS.seg[1]);
	ll ans = 0;
	auto mark_bad = [&](int val, int sgn) {
		if (!sz(by_val.at(val))) return;
		// dbg("START MARK BAD");
		if (A.at(val) > 1) {
			const int idx = A.at(val) - 1;
			if (idx < sz(by_val.at(val))) {
				LS.upd(by_val.at(val).at(0), by_val.at(val).at(idx) - 1, sgn);
			} else {
				LS.upd(by_val.at(val).at(0), SZ - 1, sgn);
			}
		}
		if (B.at(val) < sz(by_val.at(val))) {
			LS.upd(by_val.at(val).at(B.at(val)), SZ-1, sgn);
		}
		// dbg("END MARK BAD");
	};
	R0F(i,2*N) {
		...
		LS.upd(i+N-1, SZ-1, 1);
		const int val = P.at(i);
		mark_bad(val, -1);
		// dbg("HUH", val, sz(by_val));
		by_val.at(val).push_front(i);
		mark_bad(val, 1);
		assert(LS.seg[1].f >= 0);
		if (LS.seg[1].f == 0 && i < N) ans += LS.seg[1].s;
		// dbg(i, ans);
	}
	ps(ans);
}
