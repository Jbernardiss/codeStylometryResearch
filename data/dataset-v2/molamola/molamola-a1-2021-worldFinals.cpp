...
struct Frac {
	BigInt a, b;	// a / b, b > 0
	Frac() {}
	Frac(BigInt a, BigInt b) : a(a), b(b) {}
	void normalize() {
		if(b < 0) b = -b, a = -a;
	...
	Frac operator+(const Frac &rhs) const {
		Frac res = {a * rhs.b + b * rhs.a, b * rhs.b};
		res.normalize();
		return res;
	}
	Frac operator-(const Frac &rhs) const {
	...
	Frac operator*(const Frac &rhs) const {
		Frac res = {a * rhs.a, b * rhs.b};
		res.normalize();
		return res;
	}
	Frac operator/(const Frac &rhs) const {
		Frac res = {a * rhs.b, b * rhs.a};
		res.normalize();
		return res;
	}
	Frac operator*(const ll &rhs) const {
		Frac res = {a * BigInt(rhs), b};
		res.normalize();
		return res;
	}
...
int N, W, H;
vector <ll> vx;
using EQ = tuple<Frac, Frac, Frac>;
vector <pair<pll, EQ> > DV;
void add_line(ll x1, ll y1, ll x2, ll y2, ll A, ll B) {
	if(x1 == x2) return;
	if(x1 > x2) {
		swap(x1, x2);
		swap(y1, y2);
	...
	Frac a = {y2 - y1, 2 * (x2 - x1)}; a.normalize();
	Frac b = Frac(y1, 1) - a * (2 * x1); b.normalize();
	Frac c = a * (-1 * x1 * x1) + b * (-1 * x1);
	DV.emplace_back(pll(x1, x2), EQ(a * (-A - B), b * (-A - B), c * (-A - B)));
	Frac v = (a * (x2 * x2) + b * x2 + c) * (-A - B);
	DV.emplace_back(pll(x2, W), EQ(Frac(0, 1), Frac(0, 1), v));
	vx.pb(x1); vx.pb(x2);
}
...
void solve() {
	rep(i, 520) val[i] = EQ(Frac(0, 1), Frac(0, 1), Frac(0, 1));
	DV.clear(); vx.clear();
	scanf("%d%d%d", &N, &W, &H);
	ll P, Q, R, S;
	...
	for(int i = 1; i <= N; i++) {
		...
		add_line(X, Y, X + P, Y + Q, A, B);
		add_line(X + P, Y + Q, X + R, Y + S, A, B);
		add_line(X + R, Y + S, X, Y, A, B);
		sum_b += B;
		sum_a += A;
	}
	vx.pb(0); vx.pb(W);
	...
	for(auto &[seg, eq] : DV) {
		int xv1 = (int)(lower_bound(all(vx), seg.Fi) - vx.begin());
		int xv2 = (int)(lower_bound(all(vx), seg.Se) - vx.begin());
		auto &[a, b, c] = eq;
		get<0>(val[xv1]) = get<0>(val[xv1]) + a;
		get<1>(val[xv1]) = get<1>(val[xv1]) + b;
		get<2>(val[xv1]) = get<2>(val[xv1]) + c;
		get<0>(val[xv2]) = get<0>(val[xv2]) - a;
		get<1>(val[xv2]) = get<1>(val[xv2]) - b;
		get<2>(val[xv2]) = get<2>(val[xv2]) - c;
	}
	ll area = -P * S + Q * R;
	Frac ini = Frac(area, 2) * Frac(sum_b, 1);
	Frac res = Frac(0, 0);
	Frac a = Frac(0, 1), b = Frac(0, 1), vc = Frac(0, 1);
	for(int i = 0; i < szz(vx) - 1; i++) {
		a = a + get<0>(val[i]);
		b = b + get<1>(val[i]);
		vc = vc + get<2>(val[i]);
		auto c = vc + ini;
		auto get_v = [&](Frac x) { return a * (x * x) + b * x + c; };
		auto app = [&](Frac x) {
			Frac val = a * (x * x) + b * x + c;
			if(val.a < 0) val.a = -val.a;
			if(res.b == 0 || val < res) {
				res = val;
			}
		};
		app(Frac(vx[i], 1));
		app(Frac(vx[i + 1], 1));
		auto v1 = get_v(Frac(vx[i], 1));
		auto v3 = get_v(Frac(vx[i + 1], 1));
		if((v1.a > 0) != (v3.a > 0)) res = Frac(0, 1);
		if(a.a != 0) {
			Frac x = Frac(-b.a * a.b, b.b * a.a * 2);
			x.normalize();
			if(Frac(vx[i], 1) < x && x < Frac(vx[i + 1], 1)) {
				auto v2 = get_v(x);
				if((v1.a > 0) != (v2.a > 0)) res = Frac(0, 1);
				if((v3.a > 0) != (v2.a > 0)) res = Frac(0, 1);
				app(x);
			}
		}
	}
