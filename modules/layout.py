...
def layoutAnalysis(files: list[str]):
  ...
  for file in files:
    ...
    if spaceCount != 0:
      a[index, 1] = math.log(spaceCount/len(file))

    if emptyLinesCount != 0:
      a[index, 2] = math.log(emptyLinesCount/len(file))

    if commentCount != 0:
      a[index, 3] = math.log(commentCount/len(file))

    if linesLengthSum != 0:
      a[index, 4] = linesLengthSum/len(file)
